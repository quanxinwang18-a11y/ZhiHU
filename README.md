...
单元测试覆盖顾问池、非同质 Mock 表达、随机抽卡和 SQLite 用户/顾问数据隔离；Playwright 覆盖认证入口。完整主链路还经过应用内浏览器人工实测：
